...
-- Verificar si LibStub está disponible, si no, crear fallback
if not LibStub then
    _G.LibStub = {
...
if not LibJSON then
    -- Si NewLibrary retorna nil (versión duplicada), intenta acceso directo
    LibJSON = _G["LibJSON-1.0"]
    if LibJSON then
...
do
    ...
    function serializers:number(buffer)
        -- Ensure decimal separator is always a dot, regardless of locale
        buffer[#buffer+1] = tostring(self):gsub(",", ".")
    end
